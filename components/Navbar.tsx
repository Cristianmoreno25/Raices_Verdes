...
Complete Navbar including CartDropdown integration.
*/
...
import { FaBars, FaTimes, FaSearch, FaUser, FaLeaf } from 'react-icons/fa'
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    return () => { document.body.classList.remove('overflow-hidden'); window.removeEventListener('scroll', handleScroll) }
  }, [isOpen])
      ...
      <div className="container mx-auto px-2 py-2 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          {user ? (
            ...
            <>
              <Link href="/auth/login" className="px-3 py-1 rounded-full flex items-center hover:text-amber-200 transition-colors text-sm">
                <FaUser className="mr-1" /> Ingresar
              </Link>
              <Link href="/role-selection" className="px-3 py-1 rounded-full bg-amber-200 hover:bg-amber-300 text-green-800 font-medium transition-colors text-sm">
                Registrarse
              </Link>
            </>
          )}
